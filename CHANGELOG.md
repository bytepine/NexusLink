...
- feat(runtime): 独立包 MCP debug——将 `get_output_log` / `set_log_capture_filter` / `exec_command` / `capture_viewport` 从 Editor 迁到 Runtime（`FNexusRuntimeCapability`）。日志环形缓冲本就在 Runtime；`exec_command` 改走 `GEngine->Exec`（仍 Dangerous）；`capture_viewport` 用 Game 视口 `ReadPixels` 与顶层窗口。LevelEditor 面板 tab（`viewport` / `content_browser` / `scene_outliner` / `details` / `output_log` / `modes` / `world_settings`，未登记的 Tab ID 也可直接传）、`target=list` 枚举、`viewAngle` 环绕拍 Actor（PIE 中自动切 Simulate 再还原）与不开 PIE 的编辑器视口 Actor 裁切，改由新增 Editor 域 `capture_editor_panel` 承接；像素裁切/缩放/保存复用 Runtime 的 `FNexusCaptureUtils`（Public 导出），编辑器侧只留 `FNexusEditorCaptureUtils` 的面板与相机部分。独立包 Confirm 模式无编辑器确认窗，危险 cap 须 Custom / 面板会话覆盖 / `-NexusEnableDangerousCaps`
...
- feat(ui): 游戏内 MCP 调试面板——`NexusLink.Mcp panel [on|off]` 打开/关闭一层纯 Runtime Slate 视口叠加层（`NexusMcpDebugOverlay` + `SNexusMcpDebugPanel`，PIE / 独立 Game/DS 通用，不依赖 UMG 资产，随 `NEXUSLINK_WITH_SERVER` 一起被 Shipping 剔除）；上半展示 MCP 运行信息（来源/监听地址/鉴权）并提供开启/关闭/重启（与控制台命令同一套 `FNexusMcpActivation` 覆盖），下半按当前宿主可见性列出 Capability（独立包仅 Runtime，PIE 编辑器宿主 Editor+Runtime），按源码目录分组折叠、支持名称/描述过滤。新增 `UNexusLinkSettings::SessionDisabledCapabilities` + `SetSessionCapabilityEnabled()`/`ClearSessionCapabilityOverrides()`：面板勾选走会话级临时启停（判定优先级：会话禁用 > 会话启用 > 危险访问模式 > 持久 `DisabledCapabilities`），全程 Transient、不 `SaveConfig`，不影响 ini 与 Preferences 树勾选态，进程退出即失效
...
- feat(server): HTTP MCP 新增并发会话数上限设置（`MaxConcurrentSessions`，Editor Preferences「服务器」分类，默认 16，0=不限制）——达到上限时 `GetOrCreateDispatcher` 驱逐最久未活动（`LastActivityAt`）的会话，保证新客户端总能握手成功；被驱逐会话按已有的 404 `session_not_found` 处理
- feat(mcp): 危险 Capability 三模式访问——`dangerous` 访问级标签（`exec_command` / `exec_python` / `eval_runtime_lua` / `dofile_runtime_lua`）；Editor Preferences **访问模式**为全部禁用（默认，勾选记录保留但置灰不可改）/ 每次手动确认（沿用勾选，须传 `reason`，编辑器非模态窗口确认本次请求，自旋等待中继续 tick WebSocket）/ 自定义开启（逐项勾选则始终允许）。拒绝或超时 → `errorKind=user_denied` 禁止重试；`calls[]` 中一旦拒绝则本批其余危险 cap 自动跳过。升级时若已有危险 cap 启用则迁到自定义开启，避免静默关掉用户勾选。`-NexusEnableDangerousCaps` 仍会话级绕过门控
- feat(mcp): `tools/call` 慢调用可观测性——`NexusMcpDispatcher.cpp` 的 MultiTool 直调与 ToolRegistry 两条路径此前分别是耗时算完即丢（`(void)ExecDurationMs`）与仅 `Log` 级日志，收敛为统一的 `LogToolCallDuration`，耗时口径挪到 `EmitToolResult` 之后（含响应压缩/TTL注入/序列化），超过 `SlowCallThresholdMs` 升级为 `LogNexusMcpDispatcher` Warning；capability 层 `slow_call` feedback 埋点门限从「排除任何错误」放宽为只排除参数校验失败，慢的致命错误调用现在也会计入报告（`Note` 标 `(error)`）。不新增设置，反馈关闭时日志仍可用
- feat(mcp): Python 入口——危险 cap `exec_python`（走 `IPythonScriptPlugin::ExecPythonCommandEx`；`exec`/`eval` 传 `code`，`file` 传 `scriptPath`（相对 `Content/Python/`，C++ 侧校验相对路径、无 `..`、`.py` 后缀且文件存在，并回显绝对路径——UE 原生 `ExecuteFile` 按「首 token 是不是 `.py`」自动分流，路径写错会被当字面代码执行并报出与真实原因无关的 `NameError`）；`persistent` 控制 file 模式的 `FileExecutionScope`（字面代码走 `RunString`，本就共用 console 全局字典）；结构化返回 `output` / traceback `error`，`eval` 另回 `result`；输出按 200 行 + 单行 2000 字符双重截断；返回 `undoRecorded` 如实回报本次是否产生可回滚记录——基类 `Run()` 本就为其开了事务，但 UE 只记录调用过 `Modify()` 的对象，而 Python 最自然的 `obj.foo = x` 走 `NotifyMode::Never`、不触发 `PreEditChange`/`Modify`，只有 `set_editor_property` 与显式 `obj.modify()` 会，脚本混用两种写法时 Ctrl+Z 只回滚一半，比完全不能撤更危险；判定用新增的 `FNexusEditorTransaction::GetActiveRecordCount()` 取执行前后差值，不用 `ITransaction::IsTransient()`——后者依赖 `Finalize()` 后才算出的 `DeltaChange`，事务未结束时恒为「无变更」；**默认禁用**）+ 只读 `get_python_api`（内省当前引擎 `unreal` 模块成员/签名/doc 首行；`target`/`query` 经白名单校验后作为字面量嵌入固定脚本，不接受用户代码，故默认开；支持 `offset`/`limit` 分页与 `searchDoc` 按 docstring 检索；探测失败（如该 API 在本版本不存在）同样回 `engineVersion`/`pythonVersion`——这正是最需要版本信息的时刻；写 Python 前先探测）；两者 `Prerequisites: python`（CapabilitySpec §2.4 新增该枚举值）；编译期按磁盘 `.uplugin` 门控 `WITH_NEXUS_PYTHON`（仅 Editor 目标，不强制宿主启用 Python 插件）；可用性检查统一走 `FNexusPythonRuntime`，UE 5.6+ 借 `IsPythonConfigured`/`IsPythonInitialized` 区分「已配置未启用」与「已启用未初始化」并给出对应排查提示。Capability 计数 227→229
...
- chore(script): 从示例工程收回 `audit_capability_naming.py` / `audit_doc_sync.py`（按本仓 `Source/` / `docs/` 解析，不再依赖 NexusUnreal 路径）；插件 CI 与宿主 `run_e2e` 前置均跑 naming 审计；`CapabilitySpec.md` 引用改为 `scripts/audit_capability_naming.py`
- refactor(plugin): 八项零/最小行为变更结构重构——5 个单调用方 Utils 头（`NexusBehaviorTreeInspectUtils` / `NexusEditorContextUtils` / `NexusGameplayTagReferencerUtils` / `NexusSoundCueUtils` / `NexusCapabilityLegacyNames`）由 `Public/Utils` 降级到 `Private/Utils`；`HandleToolsCall` 改为复用 `EmitToolResult`（其签名改为返回 content 字节数供调用方记日志），消除与 MultiTool 路径重复的序列化分支；`NexusFeedback.cpp`（1383 行）按记录/报告职责拆分为 `NexusFeedback.cpp`（节流记录）与新增 `NexusFeedbackReport.cpp`（Issue 草稿/导出），共享助手收进新增 `NexusFeedbackInternal.h`；`FNexusCapabilityResultBuilder` 新增 `MakeCreatedEntry`/`AddCreatedEntry`，替换 53 个 `create_asset_*` cap 里重复的 name/path 结果尾巴；`search_asset` 里约 35 条同构的「按 Class 查 AssetRegistry」分支收进文件内 static 表，`blueprint`/`blendspace`/GAS 三项/UClass 回退等有特殊逻辑的分支保持原位置与顺序不变；JSON-RPC 响应信封（`MakeJsonRpcResult`/`MakeJsonRpcError`）由 `FNexusMcpServer` 与 `FNexusMcpDispatcher` 各写一份收敛为 `FNexusMcpDispatcher` 的公开 static，`SendResult`/`SendError` 改为复用（WS `payload_too_large` 响应随之从「无 id 字段」修正为规范要求的 `"id": null`），另收敛一处 HTTP/WS 共用的 UTF-8 解码 static；`manage_asset_behavior_tree`（1223 行）的路径导航/属性写入/EdGraph 同步等纯函数助手（约 400 行）搬进新增的 `Private/Utils/NexusBehaviorTreeEditUtils`；`manage_asset_blueprint` 的 `add_node` 4 类节点创建收进 `NexusBlueprintGraphUtils` 新增的 6 个写侧静态助手（`PlaceNewNode`/`ResolveGraphFunction`/`MakeCallFunctionNode`/`MakeEventNode`/`MakeVariableNode`/`MakeGenericNode`），消除节点放置样板 5 处重复。全仓净减约 700 行，不改 MCP 协议输出、不改任何 cap 的 schema 字面量
...
- fix(mcp): `get_asset_data_table` 与 `manage_asset_blueprint` 写权限缺口——`mode=rows` 现接受 `rowNames` 或 `nameFilter` 任一非空（此前无 `rowNames` 直接报错，`nameFilter` 只在 schema 分支生效），两条路径统一走 `offset`/`limit` 分页避免大表一次性导出；`manage_asset_blueprint` 的 Actor 门收窄为仅 `add_component`/`remove_component`/`set_component_property`，`set_defaults` 移出门外——此前非 Actor Blueprint（如 GameplayAbility）完全无法写自定义 CDO 属性
...
- fix(plugin): 自定义引擎不在 `{Project}/../Engine` 时可选插件探测失败（GAS/Niagara 等 `WITH_*=0`，MCP 无对应 Capability）——`CollectPluginSearchDirs` 改用 UBT `EngineDirectory`；因此编进 Paper2D 后，`manage`/`create` 改走 `FScopedFlipbookMutator` / `InitializeSprite` / `SetPivotMode`（4.26 无 `SetSourceTexture`/`SetFramesPerSecond`）。探测命中后 UBT 会报「模块依赖未在 .uplugin 声明」：磁盘仍 `Enabled: false` + `Optional: true`（不强制启用），编译期把实际链接的插件标进 UBT 依赖以消警告。跨版本：`EnhancedInputEditor`→`InputEditor`、缺 `.Build.cs` 的编辑模块跳过、Game/`BuildPlugin` HostProject 不编引擎可选插件、4.27 Niagara `SetSource` 两参
...
- **BREAKING** refactor(mcp): 全部 MCP/AI 可见文案英文化（Capability Description/WhenToUse/Schema description、Run()/Execute error、元工具、InitializeInstructions、AIRules、ProxyConfig、HTTP JSON `empty_body`/`session_not_found`）；源码注释 / 设置面板 / UE_LOG / CapabilitySpec 仍中文
...
- refactor(mcp): 新增 `FNexusActionCapability` 基类（`ExtractOperations` → `PrepareTarget` → 逐 op 分派 → `FinalizeTarget`）；`manage_asset_behavior_tree` / `manage_asset_blueprint` / `manage_asset_material` / `manage_asset_level_sequence` / `manage_asset_anim_blueprint` / `manage_asset_niagara_system` / `manage_asset_anim_sequence` / `manage_asset_user_widget` / `manage_asset_static_mesh` / `manage_asset_state_tree` / `manage_asset_meta_sound` / `manage_asset_control_rig` / `manage_asset_skeletal_mesh` / `manage_asset_movie_pipeline_config` / `manage_asset_paper_tile_map` / `manage_asset_level` / `manage_asset_anim_montage` / `manage_asset_blackboard` / `manage_asset_physics_asset` / `manage_asset_input_action` / `manage_asset_data_table` / `manage_asset_blend_space` / `manage_asset_material_parameter_collection` / `manage_asset_pcg_graph` / `manage_asset_eqs` / `manage_asset_skeleton` / `manage_asset_sound_cue` / `manage_asset_struct_field` / `manage_asset_ik_rig` / `manage_asset_curve` / `manage_asset_gameplay_ability` / `manage_asset_gameplay_effect` / `manage_asset_view_model` / `manage_asset_foliage_type` / `manage_asset_input_mapping_context` / `manage_asset_ik_retargeter` / `manage_asset_data_asset` / `manage_asset_enum` / `manage_asset_anim_composite` / `manage_asset_paper_flipbook` / `manage_asset_pose_search` / `manage_asset_data_layer` / `manage_asset_geometry_collection` / `manage_asset_attribute_set` / `manage_asset_string_table` / `manage_asset_media_source` / `manage_asset_paper_sprite` / `manage_asset_lua_binding` / `manage_asset_common_button_style` / `manage_asset_common_text_style` / `manage_asset_sound_submix` / `manage_asset_texture` / `manage_asset_sound_wave` / `manage_asset_render_target` / `manage_asset_font` / `manage_asset_sound_attenuation` / `manage_asset_physical_material` / `manage_asset_gameplay_cue_notify` / `manage_asset_sound_concurrency` / `manage_asset_sound_class` 迁入 action handler 静态分派；CapabilitySpec §2.1.1 基类选择表与 §8.9 对齐（带 `operations[]` 的新增 `manage_*` 必须继承该基类并走 `FNexusArgs`；Action handler 写 `Ctx.Entry`、子类禁止 `Execute`；手写 `ExtractOperations` 循环已收口）；README / README.en / architecture 开发路径同步该表
...
- fix(mcp): HTTP 通道线程安全——`POST /stream` 的会话表与 Dispatch、`GET /status` 的 `netRole` 均回切 GameThread；收包线程不再 `FEvent::Wait`（推迟 `OnComplete`），避免与会话清理 Ticker 并发改 `HttpSessions` 崩溃，以及关编辑器时永久等待
- fix(mcp): 设置面板 MCP Capabilities 按源码目录分组失效——`BuildDefinition`/`Out.Name` 后运行时扫描仍找 `GetName()`，全部掉进粗粒度 Tags（蓝图/数据资产/编辑器…）；改为注册期记录 `__FILE__` 相对 `Private/Capabilities` 路径（二进制安装无需源码即可多级折叠）；`SegmentToRowLabel` 补 CommonUI/ControlRig/GeometryCollection/IKRig/MetaSound/MVVM/Niagara/Paper2D/PCG/PoseSearch/Sequencer/StateTree/WorldPartition/Actor 中文映射；删除无源码目录的 `property` 段与不可达 `Private/Tools/` 分组分支
