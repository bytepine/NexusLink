...
- refactor(mcp): 新增 `FNexusArgs` 参数读取门面；Execute 删除 Schema 已覆盖的 required/空串死校验，D/E 方言改为门面调用；三元工具 `submit_feedback` / `search_capabilities` / `call_capability` 的 MCP 参数读取对齐（`calls[]` 与错误 JSON 仍原生 TryGet）
