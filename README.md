...
| 模式 | 行为 |
|---|---|
| **全部禁用**（默认） | search 报 `disabled`，调用失败。Capability 树里仍显示，强制未勾选且不可勾选 |
| **每次手动确认** | 树里默认勾选、可取消。勾选的可被 search/调用，每次执行前编辑器弹窗。AI 须传 `reason`（目的、预期效果、为何没有更安全的专用 cap）。允许仅针对**本次**；拒绝返回 `errorKind=user_denied`（不要重试）。超时（默认 90s）自动拒绝。全屏 PIE 下请先退出沉浸模式 |
