...
MCP 客户端通常把 `tools/list` + `initialize.instructions` **每模型轮次**重新注入 prompt。固定开销粗估如下（227 Capability、源码 schema 解析、chars÷4；不含 call 返回体与对话历史）：
...
| **蓝图** | Blueprint 变量/函数/图节点/连线/组件/CDO；`get` 顶层 `compileStatus`/`hasCompilerErrors`，section `orphaned`/`execPaths`；`create` 对 Actor 补 BeginPlay，`parentClass=Interface` 建 BPI；`manage` 支持 `K2Node_Event`、`add_function` / `add_interface`、`promote_pin`、`add_macro` / `add_timeline` / `add_dispatcher` / `add_local_variable`；`saveToDisk` / `compile` 可选 | 全版本 |
...
| **Paper2D** | PaperSprite / PaperFlipbook（含 create）/ PaperTileMap（含 create + 图层/格子写） | 需 Paper2D 插件 |
...
| **引擎核心小资产** | Curve（Float/Vector/LinearColor/CurveTable）、UserDefinedEnum、AnimComposite、PhysicalMaterial（含 create）、TextureRenderTarget2D、StringTable、Font（含 create；TTF 走 `reimport_asset`）、FoliageType、FileMediaSource | 全版本 |
