...
7. **Editor / Game capture**: `get_editor_context` (selection, Content Browser path); `search_console_variables`; `capture_viewport` captures Game/PIE viewport (`target=pie`) or the top-level window (`editor` / `editor_desktop`); LevelEditor panel tabs and Actor `viewAngle` shots need `capture_editor_panel` (Editor only, `target='list'` enumerates panels). Tag referencers via `get_gameplay_tags` (`referencers` + `tag`). Logs: `get_output_log` / `set_log_capture_filter` (available in Editor and standalone Game).
...
- **`exec_command` requires non-empty `command`**. Dangerous caps in Confirm mode require `reason`; `user_denied` → do not retry.
