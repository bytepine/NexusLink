...
graph TB
    ...
    subgraph UE["Unreal Engine 进程"]
        subgraph NexusLink["NexusLink 插件"]
            ...
            Caps[FNexusCapability<br>176 原子能力]
        end
    end
...
`UNexusLinkSettings::bEnableMcpServer` 为总开关，**默认 `false`**。另支持命令行 **`-EnableNexusMcp`** 与控制台 **`NexusLink.EnableMcp 1|0`**（均会话级，不写盘）；与 Preferences 为 OR。主模块 **`Type: Editor`**——仅 Editor / PIE 加载；Game / Dedicated Server / Shipping 不加载；`StartupModule` 在 `!WITH_EDITOR` 时空返回。未启用时不创建 `FNexusMcpServer`；Preferences 勾选后经 `PostEditChangeProperty` 即时启停。
...
| 能力层 | `FNexusCapability` | 176 个原子工作单元（`WITH_GAS=0` 减 10；`WITH_NIAGARA=0` 再减 1；`WITH_STATETREE=0` / `WITH_MVVM=0` 各再减 1），按域分类 |
...
固定上下文粗估（176 Capability、chars÷4）：SearchMode 每轮 ~1.4k tok，MultiTool ~17.6k tok（约 **12.6×**）；差别几乎全在 tools/list。完整对比与建议见 [README §暴露模式](../README.md#暴露模式toolslistmode)。
