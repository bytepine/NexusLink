...
|------|----------------|-------------------------|---------|
| **SearchMode**（默认） | 3 个元工具 | `InitializeInstructions.SearchMode.md`（完整路由表 + 调用规范） | AI 通过 `search_capabilities` 按需发现，降低每轮 tools/list token |
| **MultiTool** | `submit_feedback` + 全部已启用 Capability（各作独立 MCP Tool） | `InitializeInstructions.MultiTool.md`（精简全局约束） | 需要客户端一次性枚举全部能力的场景 |

固定上下文粗估（176 Capability、chars÷4）：SearchMode 每轮 ~2.3k tok，MultiTool ~17.6k tok（约 **7.6×**）；差别几乎全在 tools/list。完整对比与建议见 [README §暴露模式](../README.md#暴露模式toolslistmode)。
