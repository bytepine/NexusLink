...
启用 MCP 后：
- **编辑器标题栏右侧**（与 FPS/内存/对象同一组）显示 MCP/WS 端口号（默认开启；关闭 MCP 服务器后不显示）
...
- 可在设置中开关「在状态栏显示端口号」（**默认开启**）；关闭「启用 MCP 服务器」后不显示
...
| 响应默认值压缩 | 全工具 JSON 响应自动抽取重复字段到 `*_defaults`（缺省即默认：`{**defaults,**entry}`） |
| 自动卸载读取引入的包 | 批量只读读资产时，达阈值后整批卸载本次引入的包（默认开） |
| 卸载阈值（包数量） | 累积引入包数达此值即卸载 + GC（默认 16；0 = 只按内存） |
| 内存高水位（MB） | 进程物理内存较调用起点增长超过此值即卸载 + GC（默认 1024；0 = 关闭） |

控制台行按钮：**打开目录** / **导出 Markdown**（生成报告并归档）/ **创建 GitHub Issue**。

> 批量读取后若需保留包在内存，对 `call_capability` 传 `keepLoaded=true`；需立即释放时可调 `unload_asset`。
...
> 如果端口发生自动切换，请以 UE 编辑器菜单栏/设置面板中显示的实际端口为准。
...
| **MultiTool** | `submit_feedback` + 各已启用 Capability（各为独立 MCP Tool） | **无** `search_capabilities` / `call_capability`；直接 `tools/call` 对应 Tool 名。固定上下文约 **~21.9k tok/轮（~15.6×）**；仅当客户端必须一次枚举全 Tool 时使用，并关掉无关 Capability |
