...
bool FNexusCapabilityRegistry::CollectOperationActions(const FString& CapName, TArray<FString>& OutActions)
{
	OutActions.Reset();
	const FCapRecord* Rec = Get().FindRecordByName(CapName);
	if (!Rec || !Rec->Def.InputSchema.IsValid())
	{
		return false;
	}
