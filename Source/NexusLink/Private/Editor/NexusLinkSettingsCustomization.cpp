...
struct FNexusCapSettingsUiLayout
{
	static FString SegmentToRowLabel(const FString& Seg)
	{
		...
		if (L == TEXT("commonui")) return TEXT("通用 UI");
		if (L == TEXT("controlrig")) return TEXT("控制绑定");
		if (L == TEXT("geometrycollection")) return TEXT("几何集合");
		if (L == TEXT("ikrig")) return TEXT("IK");
		if (L == TEXT("metasound")) return TEXT("元声音");
		if (L == TEXT("mvvm")) return TEXT("视图模型");
		if (L == TEXT("niagara")) return TEXT("粒子");
		if (L == TEXT("paper2d")) return TEXT("纸片");
		if (L == TEXT("pcg")) return TEXT("程序化生成");
		if (L == TEXT("posesearch")) return TEXT("姿态搜索");
