...
void FGetAssetDataLayerCapability::BuildDefinition(FNexusCapabilityDefinition& Out) const
{
	...
	Out.Description = TEXT("Read DataLayer asset props: type (Runtime/Editor), debug color (>=UE5.1). Use manage_asset_data_layer for writes.");
	...
	Out.WhenToUse = TEXT("Read DataLayerAsset type and debug color (>=UE5.1)");
}
...
FCapabilityResult FGetAssetDataLayerCapability::Execute(const TSharedPtr<FJsonObject>& Arguments) const
{
	return FNexusCapabilityResultBuilder::Build([&](auto& OutEntries, auto& OutTop, auto& OutError)
	{
		...
		if (!DLA)
		{
			...
			ErrObj->SetStringField(TEXT("error"), TEXT("DataLayerAsset not found"));
...
FCapabilityResult FGetAssetDataLayerCapability::Execute(const TSharedPtr<FJsonObject>&) const
{
	...
	{
		OutError = TEXT("get_asset_data_layer requires UE5.1+");
	});
}
